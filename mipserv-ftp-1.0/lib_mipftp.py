...
class servidor:
  ...
  def __init__(self,conf,class_cliente=False):
	...
	if(class_cliente == False):
	  #Si no se especifica una clase derivada
	  h_cliente=cliente(sock_cliente,self)
	else:
	  h_cliente=class_cliente(sock_cliente,self)
	h_cliente.start()
...
class cliente(threading.Thread):
  ...
  def run(self):
    ...
    while(True):
      #Verifica que no se quiera salir
      if(self.servidor.detener==True):
	#print 'Deteniendo hilo...'
	self.servidor.h_socket.close()
	break
      try:	
	#Recibe los datos
	recibido=self.sock.recv(self.servidor.conf['max_size'])
	...
	for comando_y_pars in comandos:
	  ...
	  if(comando_y_pars!=''):
		...
		self.datos_data_con=(self.servidor.conf['ip_pasv'],puerto)
		try:
